...
'End Select
	...
	if CBox <> 0 then
		...
		rscatchange.Open sqlcatchange, Session("Conn"), adOpenKeyset, adLockPessimistic
	end if
	%>
...
<%  IF Not IsEmpty(Request.QueryString("SecID")) THEN 
	...
	rsmenu.Open sqlmenu, Session("Conn"), adOpenKeyset, adLockOptimistic
	
	SQLset = "select * from Main where Menuid = " & SecID & " order by Catid, sort;"
	set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sqlset, Session("Conn"), adOpenKeyset, adLockOptimistic
		
	'SELECTED SECTION
	'UNNECESSARY????  USE QUERYSTRING INSTEAD?
	SQLsec = "select * from Menuitems where Menuid = " & SecID & " ;"
	set rsm = Server.CreateObject("ADODB.Recordset")
	rsm.Open sqlsec, Session("Conn"), adOpenKeyset, adLockOptimistic
	...
	rsc.Open SQLcat, Session("Conn"), adOpenKeyset, adLockOptimistic
...
<TABLE CELLPADDING=3 CELLSPACING=1 ALIGN="CENTER" BORDER="1" BGCOLOR="">
	...
	<%DO WHILE NOT rs.EOF
		...
		rscs.Open sqlcs, Session("Conn"), adOpenKeyset, adLockOptimistic
		%>
